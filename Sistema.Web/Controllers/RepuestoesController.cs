...
namespace Sistema.Web.Controllers
{
    ...
    public class RepuestoesController : ControllerBase
    {
        ...
        {
            var repuesto = await baseDatos.Repuestos.Include(d => d.distribuidor).ToListAsync();//objeto llamado repuesto ToListAsync:obtenemos la lista del registro baseDatos de la coleccion repuestos
        ...
        {
            var repuesto = await baseDatos.Repuestos.Where(c => c.nombre == "juan").ToListAsync();//objeto llamado repuesto ToListAsync:obtenemos la lista del registro baseDatos de la coleccion repuestos
        ...
        public async Task<IActionResult> Mostrar([FromRoute] int id)//espera como parametro un id debemos enviarle la url
        {
            ...
            var repuesto = await baseDatos.Repuestos.Include(d => d.distribuidor).
        ...
        public async Task<IActionResult> Actualizar([FromBody] ActualizarViewModel model)///enviamos todo el objeto ActualizarViewModel
        {
            ...
            var repuesto = await baseDatos.Repuestos.FirstOrDefaultAsync(c => c.idrepuestos == model.idrepuestos);// baseDatos.Repuestos.FirstOrDefaultAsync: devuelve primer registro que encuentre
        ...
        public async Task<IActionResult> Crear([FromBody] CrearViewModel model)
        {
            ...
            baseDatos.Repuestos.Add(repuesto);// me agregue ese repuesto
        ...
        public async Task<IActionResult> Eliminar([FromRoute] int id)
        {
            ...
            var repuesto = await baseDatos.Repuestos.FindAsync(id);
            if (repuesto == null)
            {
                return NotFound();
            }

            baseDatos.Repuestos.Remove(repuesto);
        ...
        public async Task<IActionResult> sinstock([FromRoute] int id) {
            ...
            if (repuesto.cantidad < repuesto.stockminimo && !repuesto.correoEnviado)
            {
                ...
                if (repuesto == null)
                {//si no encuntra nada
                    Console.WriteLine("-----------------Repuesto null-----------------------");
                    return NotFound();
                }
        ...
        {            
                ...
                smtp.Credentials = new NetworkCredential("correosmantenimiento@gmail.com", "Juanmateuz19");
                // Parte 2
                MailMessage mm = new MailMessage();
                mm.IsBodyHtml = true;
                mm.Priority = MailPriority.Normal;
                mm.From = new MailAddress("juan.380172809@ucaldas.edu.co");
        ...
        {
            return baseDatos.Repuestos.Any(e => e.idrepuestos == id);
        }
    }
}